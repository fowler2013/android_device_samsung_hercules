...
char *camera_fixup_setparams(struct camera_device *device, const char *settings)
{
	...
	if (id == 0) {
		if (params.get(android::CameraParameters::KEY_RECORDING_HINT)) {
			if (!strncmp(params.get(android::CameraParameters::KEY_RECORDING_HINT), "true", 4)) {
				if (params.get(android::CameraParameters::KEY_PREVIEW_SIZE)) {
					if (!strncmp(params.get(android::CameraParameters::KEY_PREVIEW_SIZE), "800x480", 7))
						params.set(android::CameraParameters::KEY_VIDEO_SIZE, "1920x1080");
				}
			}
		}
	} else if (id == 1) {
		if (params.get(android::CameraParameters::KEY_PICTURE_SIZE)) {
			if (!strncmp(params.get(android::CameraParameters::KEY_PICTURE_SIZE), "1600x1200", 9)) {
				params.set(android::CameraParameters::KEY_JPEG_THUMBNAIL_WIDTH, "320");
				params.set(android::CameraParameters::KEY_JPEG_THUMBNAIL_HEIGHT, "240");
			}
		}
	}
